...
void doit(int fd);
void read_requesthdrs(rio_t *rp, int fd, char *requestBuffer);
...
void doit(int fd) 
{
    int is_static;
    struct stat sbuf;
    char buf[MAXLINE], method[MAXLINE], uri[MAXLINE], version[MAXLINE], requestBuffer[MAXLINE];
    ...
    read_requesthdrs(&rio, fd, requestBuffer);                              //line:netp:doit:readrequesthdrs
    ...
    if (is_static) { /* Serve static content */          
        ...
        serve_static(fd, filename, sbuf.st_size, requestBuffer);        //line:netp:doit:servestatic
    }
...
void serve_static(int fd, char *filename, int filesize, char *requestBuffer)
{
    ...
    strcat(storageFile, ".backup");
    int backupFd = Open(storageFile, O_WRONLY|O_CREAT, 0);
